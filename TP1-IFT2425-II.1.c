...
bool calculateDivergence(float x, float y)
{
  int nbrIterationMax = 200, divergenceZ = 2;
  float x1, y1, zx =x, zy = y, zn = 0;
  int i;

  for(i=0; i<nbrIterationMax; i++)
  {
    x1 = CARRE(zx) - CARRE(zy) + x;
    y1 = 2 * zx * zy + y;

    zx = x1;
    zy = y1;

    zn = sqrt(CARRE(zx) + CARRE(zy));
...
{
  int k, l;
  float x_k, y_l, z_x0, z_xk, z_y0, z_yl, z_n;

  for(k=0; k<length; k++)
  {
    for(l=0; l<width; l++)
    {
      x_k = 2.0 * (l - width/1.35)/(width - 1);
      y_l = 2.0 * (k - length/2.0)/(length - 1);

      if(calculateDivergence(x_k, y_l))
      {
        Graph2D[k][l] = 255.0;
      }
      else
...
Egalise(Graph2D,length,width,0.0);
 ...
 //Sauvegarde
 SaveImagePgm((char*)"",(char*)"FractalMandelbrotQ_II.1",Graph2D,length,width);
