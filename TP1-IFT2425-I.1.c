...
/*
  Calcul de la deriver avec l'approximation du style f'(x) = (f(x - e) - f(x)) / e 
*/
float NewtonDerivativeApprox(float cmv, float* y, float epsilon)
{
  float fxe, fx, dfx = 0.0;
  float c0, c1 = cmv;
  ...
  while(abs(c1) >= 1E-6 && abs(c1 - c0) >= 1E-6)
  {
    ...
    fxe = EstimatorCalculatorFunction((c1+epsilon), y);
    fx = EstimatorCalculatorFunction(c1, y);
