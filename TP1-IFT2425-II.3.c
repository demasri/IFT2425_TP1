...
// FONCTION DE CALCUL 
//----------------------------------------------------------
void generateMandelbrot(float** Graph2D, int width, int length) {
  ...
  for (double k=0; k < width; k += 0.1) {
    for (double l=0; l < length; l += 0.1) {
      ...
      while(i < nbrIterationMax && !(sqrt(CARRE(z_xk) + CARRE(z_yl)) > divergenceZ)) {
        ...
        if(i > 1 && (sqrt(CARRE(z_xk) + CARRE(z_yl)) > divergenceZ)){ 
          int j = 0;
          while(chemin_x[j] > 0){
            int k = chemin_x[j];
            int l = chemin_y[j];
...
//--------------------------------------------------------------------------------
 ...
 for(int i=0;i<length;i++) for(int j=0;j<width;j++) Graph2D[i][j]=j/2.0;
  ...
  printf("Temps de completion estimee : 35 secondes\n");
...
Egalise(Graph2D,length,width,0.0);
 ...
 //Sauvegarde
 SaveImagePgm((char*)"",(char*)"FractalMandelbrotQ_II.3",Graph2D,length,width);
