...
/*
  Fonction qui genere un fractal de Mandelbrot en partant de (0,0) puis se base sur les equations (2) et (3).
*/


void generateMandelbrot(float** Graph2D, int length, int width)
{
  int nbrIterationMax = 200;
  int divergenceZ = 2;
  double x_k, y_l;
  int chemin_x[nbrIterationMax], chemin_y[nbrIterationMax], color = 0;
  ...
  for (double k=0; k < width; k += 0.1) {
    for (double l=0; l < length; l += 0.1) {
      ...
      while(i < nbrIterationMax && !(sqrt(pow(z_xk, 2.0) + pow(z_yl,2.0)) > divergenceZ))
      {
        ...
        if(i > 1 && !(sqrt(pow(z_xk, 2.0) + pow(z_yl,2.0)) > divergenceZ))
        { 
          int j = 0;
          while(chemin_x[j] > 0){
            int k = chemin_x[j];
      ...
      int chemin_x[nbrIterationMax];
      int chemin_y[nbrIterationMax];
    }
}
//----------------------------------------------------------
//----------------------------------------------------------
...
int main(int argc,char** argv)
{
 ...
 flag_graph=1; // aucune visualisation necessaire pour a question I
 zoom=1;
...
//--------------------------------------------------------------------------------
 ...
 for(int i=0;i<length;i++) for(int j=0;j<width;j++) Graph2D[i][j]=j/2.0;
  ...
  printf("Generation du fractale de Madelbrot en cours...\n");
  printf("Temps de completion estimee : 2 minutes\n");

  
  //---------------------------
  //Algorithme NEWTON
  //---------------------------
  generateMandelbrot(Graph2D, 512, 512);
//--------------------------------------------------------------------------------
//---------------- visu sous XWINDOW ---------------------------------------------
//--------------------------------------------------------------------------------
...
Egalise(Graph2D,length,width,0.0);
 ...
 //Sauvegarde
 SaveImagePgm((char*)"",(char*)"FractalMandelbrotQ_II.2",Graph2D,length,width);
