...
float NewtonDerivativeApprox(float cmv, float* y, float epsilon)
{
  ...
  while(abs(c1) >= 1E-6 && abs(c1 - c0) >= 1E-6)
  {
    ...
    fx2e = EstimatorCalculatorFunction(cmv + 2*epsilon, y);
    fxe = EstimatorCalculatorFunction(cmv + epsilon, y);
    neg_fxe = EstimatorCalculatorFunction(cmv - epsilon, y);
    neg_fx2e = EstimatorCalculatorFunction(cmv - 2*epsilon, y);

    dfx = (-(fx2e) + 8*fxe - 8*neg_fxe + neg_fx2e) / (12 * epsilon);
